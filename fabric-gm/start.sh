...
./gmbin/fabric-ca-client enroll -d -u https://org0-admin:org0-adminpw@0.0.0.0:7053 --tls.certfiles /tmp/hyperledger/org0/ca/crypto/ca-cert.pem
# 在本组织中共有两个用户：orderer节点和admin用户(这里的admin和管理员是不同的。)
# 将他们注册到org0的CA服务器
./gmbin/fabric-ca-client register -d --id.name orderer1-org0 --id.secret ordererpw --id.type orderer -u https://0.0.0.0:7053 --tls.certfiles /tmp/hyperledger/org0/ca/crypto/ca-cert.pem
./gmbin/fabric-ca-client register -d --id.name admin-org0 --id.secret org0adminpw --id.type admin --id.attrs "hf.Registrar.Roles=client,hf.Registrar.Attributes=*,hf.Revoker=true,hf.GenCRL=true,admin=true:ecert,abac.init=true:ecert" -u https://0.0.0.0:7053 --tls.certfiles /tmp/hyperledger/org0/ca/crypto/ca-cert.pem
...
./gmbin/fabric-ca-client enroll -d -u https://org1-admin:org1-adminpw@0.0.0.0:7054 --tls.certfiles /tmp/hyperledger/org1/ca/crypto/ca-cert.pem
# 组织一种共有四个用户：peer1,peer2,admin,user,分别注册他们
./gmbin/fabric-ca-client register -d --id.name peer1-org1 --id.secret peer1PW --id.type peer -u https://0.0.0.0:7054 --tls.certfiles /tmp/hyperledger/org1/ca/crypto/ca-cert.pem
./gmbin/fabric-ca-client register -d --id.name peer2-org1 --id.secret peer2PW --id.type peer -u https://0.0.0.0:7054 --tls.certfiles /tmp/hyperledger/org1/ca/crypto/ca-cert.pem
./gmbin/fabric-ca-client register -d --id.name admin-org1 --id.secret org1AdminPW --id.type admin -u https://0.0.0.0:7054 --tls.certfiles /tmp/hyperledger/org1/ca/crypto/ca-cert.pem
./gmbin/fabric-ca-client register -d --id.name user-org1 --id.secret org1UserPW --id.type client -u https://0.0.0.0:7054 --tls.certfiles /tmp/hyperledger/org1/ca/crypto/ca-cert.pem
...
./gmbin/fabric-ca-client enroll -d -u https://orderer1-org0:ordererpw@0.0.0.0:7053 --tls.certfiles /tmp/hyperledger/org0/orderer/assets/ca/org0-ca-cert.pem
...
# 登录admin 用户获取msp
./gmbin/fabric-ca-client enroll -d -u https://admin-org0:org0adminpw@0.0.0.0:7053 --id.attrs "hf.Registrar.Roles=client,hf.Registrar.Attributes=*,hf.Revoker=true,hf.GenCRL=true,admin=true:ecert,abac.init=true:ecert" --tls.certfiles /tmp/hyperledger/org0/orderer/assets/ca/org0-ca-cert.pem
# 复制证书到admincerts文件夹:
...
./gmbin/configtxgen -profile TwoOrgsOrdererGenesis -channelID system-channel -outputBlock /tmp/hyperledger/configtx/system-genesis-block/genesis.block -configPath /tmp/hyperledger/configtx/

# 生成通道
export CHANNEL_NAME=mychannel
./gmbin/configtxgen -profile TwoOrgsChannel -outputCreateChannelTx /tmp/hyperledger/configtx/channel-artifacts/${CHANNEL_NAME}.tx -channelID ${CHANNEL_NAME} -configPath /tmp/hyperledger/configtx/

# 锚节点更新配置
export orgmsp=org1MSP
./gmbin/configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate /tmp/hyperledger/configtx/channel-artifacts/${orgmsp}anchors.tx -channelID ${CHANNEL_NAME} -asOrg ${orgmsp} -configPath /tmp/hyperledger/configtx/
